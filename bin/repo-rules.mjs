...
function writeRuleset(root, rules) {
  ...
  return { foreign: [...foreign].sort(), removed: removed.sort() };
}
...
function reportRemoved(names) {
  for (const name of names) {
    console.log(`removed .claude/rules/${name}: no longer generated by repo-rules`);
  }
}

function scan(root) {
  const { accepted, rejected } = validateRuleSet(detectDeclared(root));
  const { foreign, removed } = writeRuleset(root, accepted);
...
function check(root) {
  ...
  const { foreign, removed } = writeRuleset(root, live);
  reportForeign(foreign);
  reportRemoved(removed);
...
function renderOnly(root) {
  ...
  const { foreign, removed } = writeRuleset(root, rules);
  reportForeign(foreign);
  reportRemoved(removed);
