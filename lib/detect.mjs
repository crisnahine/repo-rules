...
function firstUsableLine(body) {
  return (
    body
      .split("\n")
      .find((line) => line.trim().length >= MIN_QUOTE_CHARS && /^[\x20-\x7e\t]+$/.test(line.trim())) ?? null
  );
}

// Find the quote in the source text rather than re-serialising the parsed value. A config
// written without a space after the colon, or with a different key order, still yields a
// citation that actually resolves against the file.
function quoteFromSource(body, key) {
...
function detectPackageManager(root, pkg) {
  if (pkg.json?.packageManager) {
    ...
    return rule(
      ...
      "package.json",
      pkg.body,
      quoteFromSource(pkg.body, "packageManager"),
    );
  }
...
function detectRuntimeVersion(pkg) {
  ...
  return rule(
    ...
    "package.json",
    pkg.body,
    quoteFromSource(pkg.body, "engines"),
  );
}
...
function detectCiGate(root) {
  ...
  for (const entry of entries) {
    ...
    // Quote the whole matched line, not the bare command: "pnpm lint" alone is too short to
    // survive the tripwire floor. Try each match in turn, because a block scalar step writes
    // "- run: |" as its first line, which is too short and must not veto the whole file.
    for (const match of matches) {
      const found = rule(
        "tooling.ci-gate",
        sentence(`Continuous integration runs ${shown}, declared in ${rel}.`),
        rel,
        body,
        match[0].trim(),
      );
