...
import { resolveRulesDir } from "./repo.mjs";
...
// One decision, one place. An existing store wins wherever it lives, so a repo that already
// keeps its rules outside the tree is not silently given a second, empty one; only a repo with
// no store at all has to choose, and that choice is the committed path.
export function storeDir(facts, env) {
  const resolved = resolveRulesDir(facts, env);
...
function isGenerated(path) {
  try {
    // Git for Windows defaults to core.autocrlf=true, which checks out committed LF files
    // as CRLF. Without stripping \r, the whole-line match never fires on such a checkout.
    return readFileSync(path, "utf8")
      .split("\n", MARKER_SCAN_LINES)
      .map((line) => line.replace(/\r$/, ""))
      .includes(GENERATOR_FRONTMATTER);
  } catch {
    return false;
  }
}
