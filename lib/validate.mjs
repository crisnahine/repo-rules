...
export function validateShape(rule) {
  ...
  if (typeof statement !== "string" || statement.trim() === "") {
    ...
    if (CONTROL_OR_NEWLINE.test(statement)) reasons.push("statement contains a newline or control character");
  ...
  if (!Array.isArray(rule?.evidence) || rule.evidence.length === 0) {
    ...
    for (const e of rule.evidence) {
      ...
      } else if (isAbsolute(e.path) || e.path.split(/[\\/]/).includes("..")) {
        // Citation paths come from a committed file anyone with commit access can edit.
        // Reading outside the repository is never legitimate here.
        reasons.push(`evidence path ${JSON.stringify(e.path)} must stay inside the repository`);
      }
      if (typeof e?.quote !== "string" || e.quote.trim() === "") {
        reasons.push("evidence entry is missing quote");
      } else if (e.quote.trim().length < MIN_QUOTE_CHARS) {
